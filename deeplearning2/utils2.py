...
def sample_imagenet(src, dst, nfolders=2000//25):
    src_choice = np.random.choice([os.path.join(src, f) for f in os.listdir(src)], size=nfolders, replace=False)
    for f in src_choice:
        copyanything(src=f, dst=os.path.join(dst, os.path.split(f)[-1]))
